...
### 一面（1 小时 20 分钟）
1. 自我介绍
2. 追踪完整的推广链路（进行分佣）是怎么做的呢？
- 这里主要是业务介绍，向面试官介绍一下广告投放的链路，我们主要是负责给商品的推广者分佣金的，所以推广者发什么推广信息（通常是链接）去给买家、买家下单之后佣金归属于推广者等等；
3. 推广的链接里面会带用户的信息吗？
- 这里面其实涉及好几个平台，推广者要在推广的平台上注册，所以推广者有他自己的 ID；
- 推广者想推广某个商品的时候，推广平台会把这个商品的链接再拼接上一些参数，标记这个推广者，因此这就是个专属的推广链接了；
- 推广链接参数很多、很长，因此推广平台还会去找短链平台，把长链接变成短链接再给推广者，推广者再给用户去访问；
- 用户访问短链接的时候，短链接标记这次访问并上报，用户被重定向到商品页面的时候，也会有相关上报，用户最终结算的时候也会有上报；
- 收集各种上报的信息，就可以将用户的某个订单归功于某个推广者的推广；
4. 短链接是自己做的吗，具体是怎么映射长链和跳转的？
- 比如说我是一个淘宝用户（同时我也是推广者，因为人人都可以推广），我在淘宝某个商品点击了分享宝贝至微信，那 App 就会先判断我是否具有推广者身份，如果是推广者，那它就不给普通的商品链接给我了，而是去找推广平台等一系列操作生成专属的推广链接，然后推广者把消息粘贴到微信、微博，到这里推广者的交互就结束了；
- 用户在微信、微博看到链接，点击进来，访问短链系统，短链系统做四件事：找长链映射、生成唯一 ID、上报、跳转；
- 跳转到商品页之后，商品页也有 js 脚本进行上报；
- 所有上报信息最终会进入数仓，数仓有大数据的同事负责，进行关联和归因等；
5. 这个推广的提成是按照订单维度来算的吗？
- 是的订单的奖励会结算给最后一次引导这个用户进来的推广者，也就是说用户点了很多个推广者的链接之后下单，这个单的奖励属于最后一次点击的专属链接的推广者；
6. 这种模式会存在商家作弊吗？
- 存在风险；
- 由风控团队处理。具体发生在数据采集到数仓之后，由他们团队处理完最后再把结果给回来；
7. 这个短链的长度是固定的吗，是怎么设计的？
- 不是固定的，但是都在 i ~ j 个字符长度之间；
- 尽管明文的长度会有浮动，但是二进制的长度是固定的，相当于我们生成了唯一的二进制数，再通过 base62 编码回英文 + 数字的形式；
- 二进制数则是分成了几段：有 x 位是版本号，y 位是校验位，z 位是雪花算法；雪花算法中则包含了 z1 位机器号，z2 位时间戳，z3 位自增数；
8. 跳转的时候用的是 301 还是 302 跳转？为什么？
- 302（想回答的是 301，具体看下面）；
- 不会，但应该是依靠语义上的选择，我们想表明这个 Page 是永久重定向；
9. 短链数据为什么从 TiDB 迁移到 MySQL？
- （公司层面）全局部署资源的问题、资源共用的问题；
- （业务层面）性能问题；
10. 短链迁移到 MySQL 做的分库分表依据是什么？
...
11. 双写的时候怎么保证 TiDB 和 MySQL 数据一致？
- 没有办法严格保证；
...
14. 如果两个系统间的链路太长，数据变更时间很长或者数据丢了，那核对系统要以哪边的数据为准（去修改另一个系统）呢？
- 核对系统仅做核对和告警，不进行数据回填、回刷等修数据操作；
15. OpenTracing 这个是基架做的吗，为什么业务团队也要去搭呢？
...
- 业务观测到的性能损失微乎其微；
17. Tracing 有一定的采样率，如果调用链路中某个服务有报错，它并不一定能被采样到。怎样能使报错的调用都被采样到呢？
